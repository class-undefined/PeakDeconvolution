...
def train(
    ...
    preprocessor.ax.plot(
        X.cpu().detach().numpy(),
        model(X).sum(dim=0).cpu().detach().numpy(),
        label="Fitted",
    )
...
def test1():
    p = DataPreprocessor.from_text("datas/UV_Vis_TwoPeak.txt")
    train(preprocessor=p, epochs=200, lr=50)


def test2():
    p = DataPreprocessor.from_csv("datas/test-deconvolve.csv").smooth()
    train(preprocessor=p, epochs=500, batch_size=28, lr=0.001, device="cpu", seed=50)


def test3():
    p = DataPreprocessor.from_csv("datas/test-deconvolve.csv")
    train(
        preprocessor=p,
        num_peaks=2,
        epochs=500,
